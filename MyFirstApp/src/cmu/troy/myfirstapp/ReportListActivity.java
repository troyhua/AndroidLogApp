...
/**
 * An activity representing a list of Reports. This activity has different presentations for handset
 * and tablet-size devices. On handsets, the activity presents a list of items, which when touched,
 * lead to a {@link ReportDetailActivity} representing item details. On tablets, the activity
 * presents the list of items and item details side-by-side using two vertical panes.
 * <p>
 * The activity makes heavy use of fragments. The list of items is a {@link ReportListFragment} and
 * the item details (if present) is a {@link ReportDetailFragment}.
 * <p>
 * This activity also implements the required {@link ReportListFragment.Callbacks} interface to
...
public class ReportListActivity extends FragmentActivity implements ReportListFragment.Callbacks {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (findViewById(R.id.report_detail_container) != null) {
      ...
      ((ReportListFragment) getSupportFragmentManager().findFragmentById(R.id.report_list))
  ...
  /**
   * Callback method from {@link ReportListFragment.Callbacks} indicating that the item with the
